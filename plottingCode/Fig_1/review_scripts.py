...
all_names = [ 'GWs', \
                       'isolated binaries',  'CHE','pop-III','triples', 'dynamical: GC', 'dynamical: NC',  'dynamical: YSC']
colors = sns.color_palette("husl", len(all_names))   
name_colors = dict(zip(all_names, colors))
name_labels = [ r'Gravitational waves', \
                       r'Isolated binary evolution',  r'Chemically homogeneous evolution', r'Population III stars', r'Triples', r'Globular clusters', r'Nuclear star clusters',  r'Young/Open stellar clusters']
...
def plot_using_plotting_style(axe, ps, x_, y_, color):
    ...
    if ps in [3,4,5,6, 7, 8,9,10,11,13, 14, 15, 17, 18 ]:
        axe.errorbar(x=x_, y=y_, color=color, zorder=5, lw=5.5)#, ecolor=color)
        # axe.plot(x=x_, y=y_, fmt='o', zorder=1E5, lw=3.5, ecolor='k', color='k')
        if ps==4:
            # plot center values
...
def plot_Broekgaarden_rates(axe, v_height):
    MSSFRnameslist = []
    ...
    TypeColours = ['#66c2a5',  'lightskyblue', 'orange' ]
    ...
    for ind_L, MSSFRname in enumerate(MSSFRnameslist):
        ...
        maskBHBH = (rate_BHBH>= BHNSrate0[0]) & (rate_BHBH <=BHNSrate0[1])
        maskBHNS = (rate_BHNS>= GW200115[0]) & (rate_BHNS <=GW200115[2])
        maskNSNS = (rate_NSNS>= GW200105[0]) & (rate_NSNS <=GW200105[2])

        maskGWTC2 = (maskNSNS==1) & (maskBHNS==1) & (maskBHBH==1) 
        NSBH_masked_rates.append(rate_BHBH[maskGWTC2])

        axe.scatter(rate_BHBH[maskGWTC2],  v_height*np.ones_like((np.array(range(len(rates))))[maskGWTC2]), s=145, c='k', marker='x', zorder=1E8)
        ...
        super_mask = (maskGWTC2_DCO==1) & (maskGWTC2==1)
        
        
        axe.scatter(rate_BHBH[super_mask], v_height*np.ones_like(np.array(range(len(rates)))[super_mask]),  s=145, c='r', marker='x', zorder=1E9)

        # axe.scatter(NSBH_masked_rates,  v_height*np.ones_like(NSBH_masked_rates), s=145, c='k', marker='x', zorder=1E8)
        # axe.scatter(super_masked_rates, v_height*np.ones_like(super_masked_rates), s=145, c='r', marker='x', zorder=1E9)
...
#             colum_list = df.columns
        ...
        for ind_n, name in enumerate(colum_list):
            ...
            # if name == 'GWs: masses like detections, Abbott et al. (2021)':
            #     r_color = '#66c2a5'
            # elif name == 'GWs: broad mass distribution, Abbott et al. (2021)':
            #     r_color = 'lightgray'

            r_color = 'lightgray'
            if labelname == 'isolated binaries':
                if name == 'Broekgaarden et al. (2021)':
                    r_color =  '#66c2a5'
            elif labelname =='GWs':
                r_color =  r_color=df_colordict[labelname]
            else:
                r_color = 'lightgray'
            #     r_color=df_colordict[labelname]


            if name == 'Broekgaarden et al. (2021)':
                plot_Broekgaarden_rates(axe, v_height=v_height)


            if ps in range(20):
                plot_using_plotting_style(axe, ps, x_=np.asarray(rate), y_=v_height*np.ones_like(rate), color=r_color)
            else: 
                plot_using_plotting_style(axe, ps=3, x_=np.asarray(rate), y_=v_height*np.ones_like(rate), color=dr_color)
            ...
            if name == 'Broekgaarden et al. (2021)':
                axe.text(max(rate)*1.25, v_height, s=r'$\leftarrow \textbf{this work}$', ha='left', va='center', fontsize=fs+3)
            ...
            if labelname == 'isolated binaries':
                ...
                l_color =  r_color=df_colordict[labelname]
            ...
            if ind_n==(0): #len(df.columns)-1
                ...
                if switchLabelLeft==True:
                    axe.text(2*1E-3, v_height+0.3, s=r'\textbf{%s}'%names_label_dict[dict_name] , rotation = 0, fontsize = fs+2, color=l_color, ha = 'left', va='center', weight = 'bold')
                else:
                    axe.text(8*1E3,  v_height+0.3, s=r'\textbf{%s}'%names_label_dict[dict_name] , rotation = 0, fontsize = fs+2, color=l_color, ha = 'right', va='center', weight = 'bold')
...
#                     switchLabelLeft = True 
        ...
        #     combined_rates = np.concatenate((combined_rates, [min_, max_]))
            ...
            if df_labels[ind_file]!='kilonovae':
                plot_using_plotting_style(axe, ps=ps, x_=np.asarray(rate), y_=v_height*np.ones_like(rate), color=df_colordict[df_labels[ind_file]])
